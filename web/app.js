...
const SPREADS = {
    "Single Card": { id: "single-card", positions: [{ id: "focus", label: "Focus" }] },
    "Three-Card": { id: "three-card", positions: [
        { id: "past", label: "Past" }, { id: "present", label: "Present" }, { id: "future", label: "Future" }
    ] },
    "Five-Card — Situation & Advice": { id: "five-situation", positions: [
        { id: "situation", label: "Situation" }, { id: "challenge", label: "Challenge" }, { id: "root", label: "Underlying Cause" }, { id: "guidance", label: "Guidance" }, { id: "outcome", label: "Likely Direction" }
    ] },
    "Five-Card — Five-Card Cross": { id: "five-cross", positions: [
        { id: "present", label: "Present" }, { id: "past", label: "Past Influence" }, { id: "future", label: "Future Influence" }, { id: "foundation", label: "Foundation" }, { id: "potential", label: "Potential" }
    ] },
    "Five-Card — Expanded Past–Present–Future": { id: "five-expanded", positions: [
        { id: "past", label: "Past" }, { id: "present", label: "Present" }, { id: "future", label: "Future" }, { id: "advice", label: "Advice" }, { id: "outcome", label: "Outcome" }
    ] },
    "Five-Card — Relationship Cross": { id: "five-relationship", positions: [
        { id: "you", label: "You" }, { id: "other", label: "Other Person" }, { id: "foundation", label: "Relationship Foundation" }, { id: "present", label: "Present State" }, { id: "outcome", label: "Likely Outcome" }
    ] },
    "Five-Card — Decision / Crossroads": { id: "five-decision", positions: [
        { id: "current", label: "Current Position" }, { id: "path-a", label: "Path A" }, { id: "path-b", label: "Path B" }, { id: "hidden", label: "Hidden Influence" }, { id: "guidance", label: "Guidance" }
    ] },
    "Five-Card — Timeline": { id: "five-timeline", positions: [
        { id: "recent", label: "Recent Past" }, { id: "present", label: "Present" }, { id: "near", label: "Near Future" }, { id: "developing", label: "Developing Future" }, { id: "outcome", label: "Outcome" }
    ] },
    "Celtic Cross": { id: "celtic", significator: true, positions: [
        { id: "covers", label: "What Covers" }, { id: "crosses", label: "What Crosses" }, { id: "crowns", label: "What Crowns" }, { id: "beneath", label: "What Is Beneath" }, { id: "behind", label: "What Is Behind" }, { id: "before", label: "What Is Before" }, { id: "self", label: "Himself / Herself" }, { id: "environment", label: "The House / Environment" }, { id: "hopes", label: "Hopes or Fears" }, { id: "outcome", label: "What Will Come" }
    ] }
};
...
function showMenu() {
    ...
    cardImage.src = CARD_BACK; cardImage.alt = "Tarot card back"; currentCard.classList.remove("reversed"); cardInterpretation.classList.add("hidden"); refreshPositionTracker(); refreshReadingLog();
}
...
function showSignificatorBack() {
    ...
    cardImage.src = CARD_BACK; cardImage.alt = "Significator card back"; currentCard.classList.remove("reversed");
    instruction.textContent = "Tap the card to reveal the Significator"; cardInterpretation.classList.add("hidden"); refreshPositionTracker(); refreshReadingLog();
}
function revealSignificator() {
    state.significatorRevealed = true; state.currentRevealed = true;
    cardImage.src = cardImagePath(state.significator); cardImage.alt = `${state.significator} (${state.significatorOrientation})`;
    currentCard.classList.toggle("reversed", state.significatorOrientation === "Reversed");
    ...
    state.readingLog = [{ position: "Significator", card: state.significator, orientation: state.significatorOrientation }];
    refreshPositionTracker(); refreshReadingLog();
}

function showCard() {
    const position = state.currentPositions[state.cardIndex]; if (!position) return;
    state.currentRevealed = false;
    readingInfo.textContent = `${state.currentSpreadName} • ${position.label} • Card ${state.cardIndex + 1} of ${state.currentCards.length}`;
    cardImage.src = CARD_BACK; cardImage.alt = "Tarot card back"; currentCard.classList.remove("reversed"); instruction.textContent = "Tap the card to reveal";
    cardInterpretation.classList.add("hidden"); refreshPositionTracker(); refreshReadingLog();
}
...
function revealCard() {
    ...
    cardImage.src = cardImagePath(cardName); cardImage.alt = `${cardName} (${orientation})`;
    currentCard.classList.toggle("reversed", orientation === "Reversed"); state.currentRevealed = true;
    ...
    interpretationText.textContent = interpretation || "This position does not yet have a contextual interpretation.";
    cardInterpretation.classList.remove("hidden");
    state.readingLog.push({ position: position.label, card: cardName, orientation });
    refreshPositionTracker(); refreshReadingLog();
}

function nextCardOrMenu() {
    state.cardIndex += 1;
    if (state.cardIndex >= state.currentCards.length) { showMenu(); return; }
    showCard();
}

function refreshPositionTracker() {
    if (!positionTracker) return;
    if (!state.currentPositions.length) { positionTracker.innerHTML = ""; return; }
    positionTracker.innerHTML = state.currentPositions.map((position, index) => {
        const entry = state.readingLog.find(item => item.position === position.label);
        const active = index === state.cardIndex ? " active" : "";
        const revealed = entry ? " revealed" : "";
        const detail = entry ? `${escapeHtml(entry.card)} (${escapeHtml(entry.orientation)})` : "Not yet revealed";
        return `<div class="position-item${active}${revealed}"><span class="position-number">${index + 1}</span><span class="position-name">${escapeHtml(position.label)}</span><span class="position-card">${detail}</span></div>`;
    }).join("");
}

function refreshReadingLog() {
    if (!state.readingLog.length) { readingLog.innerHTML = "<em>Revealed cards will appear here</em>"; return; }
